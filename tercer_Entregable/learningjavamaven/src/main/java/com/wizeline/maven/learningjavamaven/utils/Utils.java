...
public class Utils {
	...
	/**
	 * ^ Indica el inicio de la declaracion (?=.*[0-9]) Debe contener un digito del
	 * 1 al 9 (?=.*[a-z]) Debe contener una letra minuscula (?=.*[A-Z]) Debe
	 * contener una letra mayuscula (?=.*[@#$]) Debe contener un caracter especial
	 * de los indicados entre corchetes {6,8} Debe tener una longitud de entre 6 a 8
	 * caracteres $ Indica el final de la declaracion
	 */
	private static final String PASSWORD_PATTERN = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$]).{6,8}$";
	private static final Pattern pattern = Pattern.compile(PASSWORD_PATTERN);

	// diseño de patron para validar fecha
